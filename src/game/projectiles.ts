...
function checkThingCollision(proj: Projectile, newX: number, newY: number, newZ: number): boolean {
  ...
  for (const obj of mapObjs) {
    ...
    const killed = damageMobj(obj, damage);
...
function checkPlayerCollision(
  _proj: Projectile,
  _newX: number, _newY: number, _newZ: number
): boolean {
  // In original DOOM, the player's own missiles don't directly hit the player
  // (source check). Splash damage is handled by P_RadiusAttack in explodeProjectile.
  return false;
}
